...
import com.suitebde.models.ensisa.*
            ...
            try {
                ...
                for (frame in it.incoming) {
                    ...
                    if (text.startsWith("ChatMessage:")) {
                        onMessage(Serialization.json.decodeFromString<ChatMessage>(text.substring("ChatMessage:".length)))
                    } else if (text.startsWith("ChatMembership:")) {
                        onMessage(Serialization.json.decodeFromString<ChatMembership>(text.substring("ChatMembership:".length)))
                    }
                }
            } finally {
                onDisconnected()
